...
| 149 | §10x.156 + §10x.157 + §10x.158 | User: *"For Laguna there are 4 images user upload: image 1 RHB security with lot/PTD; image 2 lawyer attestation; image 3 SPA schedule (purchaser address misleading in singapore — AI need to interpret SPA schedule correctly); image 4 Land charge form (Jalan rimbun is residence not pledged property — AI need to understand context). address could mean place of residence not asset address. Understanding the form and context is important"*. Vision was extracting party-residence addresses (chargor's, purchaser's, lawyer's office) into `property_address`, then matcher token-failed because "Jalan Institusi" (chargor) doesn't share tokens with "Jalan Sri Laguna" (subject). Sri Laguna's actual title doc `7a4d55ae` (Charge form, title=431161 lot=337203 mukim=Pulai) couldn't bind. Same for `584138c3` SPA Schedule (title=431161 lot=135402 mukim=Pulai, purchaser address `10 Marsiling Lane Singapore`). Result: probate-required title No. shown as missing even though the data was inside the uploads. | THREE structural gaps: (1) Vision prompt collapsed all addresses into one `property_address` field with no awareness that SPA / Charge / Loan forms have a SUBJECT PROPERTY block (Schedule / Security / Charged Land) separate from PARTY ADDRESS recitals (Purchaser/Chargor/Borrower residence, Lawyer office). (2) Matcher's `_score_pair` had no tolerance for OCR field-swap — when vision puts the Geran into `lot_number` and the charge account No. into `title_number` (common on Borang 16A where multiple numeric IDs appear close together), direct identifier matching missed the swap. (3) When AI Summary has NO identifiers (user typed only an address) but DocGroup HAS identifiers + matching mukim, score was below CANDIDATE_THRESHOLD → silent residual instead of surfacing as MEDIUM candidate-with-confirm card per §10he Step 4. | Shipped commit TBD. **Three coordinated fixes**: (a) §10x.156 — `ai/unified_vision.py` Property rules section rewritten with explicit SPA/Charge/Loan/Cukai/Lawyer guidance: "the address printed on the doc is OFTEN a party's residence, NOT the subject property... Only fill `property_address` from a section that explicitly identifies the SUBJECT PROPERTY (heading: SCHEDULE / DESCRIPTION / SECURITY / CHARGED LAND / PERIHAL TANAH / OBJECT OF CHARGE)." Plus NEW field `_party_addresses: [{role, address}]` to capture chargor/purchaser/lawyer addresses separately for audit. Plus title_number guidance: "Folio N or Vol N is a folio-location WITHIN a register, NOT a title number — set title_number='' and put it in property_description." (b) §10x.157 — `services/asset_pipeline.py::_score_pair` adds OCR field-swap tolerance: when neither direct lot_match nor title_match fires, try `(ai_lot vs g_title)` and `(ai_title vs g_lot)` swaps; accept at 0.8× weight. (c) §10x.158 — new score component `identifier_doc_with_mukim_match` (+15) fires when AI Summary lacks identifiers but DocGroup has lot+title AND mukim agrees. Combined with `mukim_match` (20), reaches score 35 → above CANDIDATE_THRESHOLD (30) → surfaces as MEDIUM candidate-with-confirm card so user can confirm "yes this Charge form is for Sri Laguna" without auto-binding (preserves §10he Step 5 "NEVER guess"). **Effect**: existing extracted_data still has wrong addresses (until vision re-runs on next upload), but matcher now uses lot/title/mukim as primary signals; new uploads will avoid the property_address contamination at extraction time. Old data can be re-extracted by clearing VisionExtractCache for the affected docs. | HIGH (shipped — pending re-extract + Chrome-MCP verify on Sri Laguna title surfacing) |
