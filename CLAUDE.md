...
| 93 | §10x.120 | One client (KOID) accumulated **4 Will rows** during testing: 1 empty approved + 1 generated (Joshua) + 1 approved (LIM LAY CHENG, wrong) + 1 latest draft (correct). Wizard's `session.will_id` pinned to the wrong-data approved snapshot; chat walker wrote to the latest draft; user saw stale "LIM LAY CHENG" in wizard. The mistaken workaround I suggested was "switch wills via Saved Wills" — wrong, the bug is multiple wills existing at all. | (a) "Approve" or "Generate" actions create a new Will row with frozen step5_data instead of changing status on the same row. (b) Walker's `Will.query.filter_by(client_id=cid, status='draft').order_by(updated_at.desc()).first()` finds the LATEST draft but doesn't enforce singleton. (c) Wizard session never auto-rebinds to the latest active draft when chat creates one. So the wizard view + the chat-saved data could diverge silently. | TODO (open). Hard rule needed: **one active Will per client at any time** (active = status in {draft, generated, approved}). Approve/Generate must mutate status on the existing row (or move the previous row to a `history` table), never INSERT a new row. Walker + wizard + generator must all resolve `client_id → ONE will`. Add fail-loud check at every Will write site: if `Will.query.filter(client_id=cid, deleted_at IS NULL, status.in_(['draft','generated','approved'])).count() > 1` → raise `ContractViolation`. | LOW (open — to investigate after current fresh KOID re-run; user has deleted multi-will state) |
