...
| 81 | §10x.179 | User: "white patch on the AI chat" (Screenshot 7:41 PM). The "New chat update / Chat updated identities. Refresh to see latest." notification toast at bottom-right of the wizard page has an empty white field/box between the body text and the Dismiss button — looks like a blank patch. | The toast template renders an `<input>` or `<div>` placeholder that's intended for additional context (e.g. count of new identities) but the data field is empty so the input renders as a blank white box. | TODO: locate the toast component (search for "New chat update" / "Chat updated identities" in templates + chat.js). Either populate the missing context field with the actual update detail, or remove the blank `<input>`/`<div>` element when the data is empty. | LOW (cosmetic — toast still dismissable, but visually broken) |
