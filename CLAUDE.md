...
| 163 | §10x.223 + §10x.224 + §10x.225 | User: *"make sure this thing is functioning properly and bug free. you need to fill in some missing fields not in the whatsapp message like testator address, occupation"*. Autonomous walker test demonstrated chat SKIPPED Step 2 entirely — after IC confirmations it jumped straight to Step 5 (Beneficiaries), never asking testator's address or occupation. step1_data remained empty so gen_will_v2.py crashed with `pydantic.ValidationError: Full name is required` until patch_step1_exec.py was run manually. | THREE compounding bugs. (A) Planner Step 2 gate at `ai/chat_planner.py:401-419` only emitted the card when step1_data.full_name was truthy OR a Person.relationship='Testator' row existed. KOID test fixture has NEITHER — his own IC was never extracted as a 'nric' Document (only the family ICs). The Client table has `full_name='Koid Beng Sun'` + `nric_passport='631204-07-5743'` but the planner ignored it → card never rendered. (B) `_step2_question` showed `Occupation: _(optional — omitted)_` and offered ✓ Confirm whenever address was present — never asked the user for their occupation. The WhatsApp message almost never contains occupation, so it stayed blank in every will. (C) Walker dispatch chain didn't drive any occupation-related quickreplies. | Shipped: (a) §10x.223 — Step 2 gate fallback to Client table + IC Document harvest. When step1_data is empty and no Testator Person exists, the planner now reads `Client.full_name` + `Client.nric_passport`, scans the testator's own IC Document (by NRIC match) for extracted address/DOB, and AUTO-CREATES the Testator Person row idempotently. Downstream handlers (_try_save_testator_address / _try_confirm_testator) keep working unchanged. (b) §10x.224 — `_step2_question` now prompts for occupation explicitly AFTER address is set. 8 quickreply options: Retired / Engineer-Professional / Director / Self-employed / Employee / Homemaker / Other-type / Skip-occupation. ✓ Confirm shows only when both address AND (occupation OR `_occupation_skipped=True`) are resolved. `_try_save_testator_address` handles the new `occupation skip` quickreply by stamping `_occupation_skipped=True` on step1_data. `_try_confirm_testator` blocks Confirm with `testator_occupation_required` if occupation missing AND not skipped. (c) §10x.225 — `_try_confirm_testator` now preserves `occupation` + `_occupation_skipped` in step1_data on confirm (removed duplicate 'gender' key in s1.update). Walker patched in docker volume to drive occupation prompts (pick `occupation: Retired` default + `confirm`). | HIGH ✓ (deployed + walker end-to-end PASS — step1_data has full_name + nric + address + dob + gender + marital_status + occupation; will generation no longer needs patch_step1_exec.py) |
