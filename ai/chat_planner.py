...
def _summarise_message(raw_text: str, *, doc_fields: list = None) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            "🔥 §10x.77 — STATE FACTS ONLY. Do NOT ask follow-up questions. "
            "Do NOT include a 'Key Flags', 'Issues Requiring Clarification', "
            "or 'Follow-up' section. Do NOT prompt the user to confirm, "
            "verify, or clarify anything. Clarifications happen LATER in "
            "the per-step walkthrough cards (Step 1 IC → Step 2 Testator → "
            "Step 3 Executor → Step 5 Beneficiaries → Step 6 Specific "
            "Gifts). The summary's only job is to mirror back what was "
            "communicated and what was deduced, so the user can verify at "
            "a glance. Output exactly two sections — nothing after.\n\n"
